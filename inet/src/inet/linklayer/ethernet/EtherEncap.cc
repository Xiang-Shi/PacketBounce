...
void EtherEncap::processFrameFromMAC(EtherFrame *frame)
{
    ...
    //SHI: if sender received passed back frames
    if( strcmp(getParentModule()->getFullName(),"sender1")==0 ||strcmp(getParentModule()->getFullName(),"sender2")==0 ||strcmp(getParentModule()->getFullName(),"sender3")==0)
    {
        if(frame->getPassBackNum()>0 )
        {
            ...
            //send the passed-back frame into data queue
            send(frame, "lowerLayerOut");
        }
    }
    else
