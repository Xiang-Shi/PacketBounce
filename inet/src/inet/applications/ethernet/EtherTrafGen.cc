...
EtherTrafGen::EtherTrafGen()
{
    ...
    pauseInterval = nullptr;
    numPacketsPerBurst = nullptr;
    numPacketsPerSend = nullptr;
...
void EtherTrafGen::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        pauseInterval = &par("pauseInterval");
        numPacketsPerBurst = &par("numPacketsPerBurst");
        numPacketsPerSend = &par("numPacketsPerSend");
        numPacketsPerSendCounter = numPacketsPerSend->longValue();
        ...
        if (isNodeUp() && isGenerator())
            scheduleNextPacketForSend(-1);
    }
}


//SHI: When received passed back frames, suspend the sender

//You can send() messages to other modules, or use scheduleAt()+cancelEvent()
//to implement delays, timers or timeouts.

//SHI: Send one packet, then burst next after send interval. repeat this SEND process
//SHI: PAUSE between two SEND process
void EtherTrafGen::handleMessage(cMessage *msg)
{
    ...
    if (msg->isSelfMessage()) {
        ...
        if((numPacketsPerSendCounter--) == 0)
        {
            numPacketsPerSendCounter = numPacketsPerSend->longValue();
            scheduleNextPacketForPause(simTime());
        }
        else
            scheduleNextPacketForSend(simTime());
...
/**
 * SHI:calculate correct timerMsg value, decide when to send a self message to trigger next send
 */
void EtherTrafGen::scheduleNextPacketForPause(simtime_t previous)
{
    simtime_t next;
    if (previous == -1) {
        ...
        //SHI: add pause interval
        next = previous + pauseInterval->doubleValue();
        timerMsg->setKind(NEXT);
    }
    if (stopTime < SIMTIME_ZERO || next < stopTime)
        scheduleAt(next, timerMsg);
}
void EtherTrafGen::scheduleNextPacketForSend(simtime_t previous)
{
    simtime_t next;
    if (previous == -1) {
        next = simTime() <= startTime ? startTime : simTime();
